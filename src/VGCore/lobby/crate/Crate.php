...
class Crate {
    ...
    private static $crate2 = [138, 7, 107];
    private static $crate3 = [133, 7, 109];
    ...
    public static function setTitle(Vector3 $pos, $title = "§k-- §r§e§lCRATE §k--", $text = "§c-----") {
        $server = new VGServer($plugin);
        $servercheck = $server->checkServer();
        if ($servercheck === "Lobby") {
            $cratetext = [self::$crate1text, self::$crate2text, self::$crate3text];
            $level = $plugin->getServer()->getLevelByName("Sam2");
            $floatingent = new FloatingTextParticle($pos, $text, $title);
